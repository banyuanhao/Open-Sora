...
def main():
    ...
    # 从配置文件中获取输出根目录，如果没有配置则使用默认值
    output_root = cfg.get("output_root", "/data/xuchenheng/outputs")
    ...
    if vae is not None:
        ...
        # 直接使用原始视频尺寸，或者指定一个固定的 latent size
        input_size = (dataset.num_frames, *dataset.image_size)
        latent_size = input_size  # 或者使用配置中指定的值
        vae_out_channels = cfg.get("vae_out_channels", 3)  # 使用原始视频的通道数
    ...
    model = (
        build_module(
            ...
            enable_layernorm_kernel=False,  # 禁用 FusedLayerNorm
            from_pretrained=None,  # 禁用预训练模型加载
        )
        .to(device, dtype)
        .train()
    )
            ...
            for step, batch in pbar:
                ...
                with timers["ckpt"] as ckpt_t:
                    if ckpt_every > 0 and (global_step + 1) % ckpt_every == 0:
                        # 创建保存检查点的文件夹
                        ckpt_dir = os.path.join(exp_dir, f"checkpoint_{global_step+1}")
                        if coordinator.is_master():
